          ...
          <div className="divide-y divide-card-border">
            {!resultados || resultados.length === 0 ? (
              ...
              resultados.map((res: any) => (
                <Link href={`/exam/${res.examen_id}/resultado?res=${res.id}`} key={res.id} className="block group">
                  <div className="p-4 sm:p-6 md:p-8 flex flex-col sm:flex-row gap-4 sm:gap-6 justify-between items-center group-hover:bg-card-hover transition-colors">
                    <div className="flex-1 w-full text-center sm:text-left">
                      <h4 className="font-bold text-base sm:text-lg text-foreground mb-1 group-hover:text-indigo-600 transition-colors">{res.examenes?.titulo || 'Examen desconocido'}</h4>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div className="hidden sm:flex text-indigo-500 opacity-0 group-hover:opacity-100 transition-opacity">
                        <span className="text-sm font-bold pr-1">Ver Detalles</span>
                        <span>→</span>
                      </div>
                    </div>
                  </div>
