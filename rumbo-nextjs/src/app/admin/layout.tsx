...
import { LayoutDashboard, FileText, BarChart3, LogOut, ShieldCheck, Users, UserCircle } from 'lucide-react'
import SidebarShell from '../components/SidebarShell'

export const metadata = {
  title: 'Panel de Administración | Aprendamos Jugando',
}
...
export default async function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const sidebarContent = (
    <>
      <div className="h-16 flex items-center px-6 border-b border-white/10 shrink-0">
        ...
        <h1 className="font-bold text-lg text-white tracking-wide">Portal Admin</h1>
      </div>

      <nav className="flex-1 px-4 py-6 flex flex-col gap-2 overflow-y-auto">
        <p className="px-2 text-xs font-bold uppercase tracking-wider text-slate-500 mb-2">Principal</p>
        <Link href="/admin" className="flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-white/10 hover:text-white transition">
          <LayoutDashboard size={18} /> Resumen
        </Link>
        <Link href="/admin/examenes" className="flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-white/10 hover:text-white transition group relative">
        ...
        <p className="px-2 text-xs font-bold uppercase tracking-wider text-slate-500 mt-6 mb-2">Organización</p>

        <Link href="/admin/usuarios" className="flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-white/10 hover:text-white transition group relative">
          <Users size={18} /> Usuarios
          <div className="absolute right-2 opacity-0 -translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 transition-all text-blue-400">
             &rarr;
          </div>
        </Link>

        <p className="px-2 text-xs font-bold uppercase tracking-wider text-slate-500 mt-6 mb-2">Análisis</p>
        <Link href="/admin/estadisticas" className="flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-white/10 hover:text-white transition">
  ...
  return (
    <SidebarShell sidebar={sidebarContent} headerLabel="Admin › Workspace">
      {children}
    </SidebarShell>
  )
}
